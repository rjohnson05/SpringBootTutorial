...
Before doing that, let's add a couple of convenience constructors and
methods to our JPA and form models.

First, in the JPA Login model, let's add a constructor that takes two
String, one the username, and the second the raw string password that
converts it to a hashed value, along with a new setter that does the
conversion. Note, if we add a constructor to either of classes, we need
to make sure we add a no-arg constructor, which is used internally in
the framework to work properly.
...
method name not associated with the class properties/fields.

We'll do something similar to the web form LoginForm, but in this case,
we don't have to worry about the hashing, since the user will always
providing unhashed passwords.
...
That should make the testing code a bit shorter.
...
Note, it's much simpler than the production setup.
...
Congratulations! At this point, we're doing with this initial Spring
